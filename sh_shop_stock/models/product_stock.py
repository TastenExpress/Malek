...
class ProductTemplate(models.Model):
    ...
    def _compute_expiry(self):
        for rec in self:
            for variant in rec.product_variant_ids:
                ...
                lot_ids = self.env['stock.production.lot'].search([('id','in',quant_ids.lot_id.ids),('expiration_date','!=',False)])
                # expiry = min(list(lot_ids.expiration_date))
                if lot_ids:
                    lst = []
                    for lot in lot_ids:
                        lst.append(lot.expiration_date)
                    expiry = min(lst)
                    rec.expiry_lot_date = expiry
                    # raise UserError(expiry)
    ...
    expiry_lot_date = fields.Date("Expiry Lot Date", compute="_compute_expiry")
