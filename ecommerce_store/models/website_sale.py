        ...
        try:
            ...
            if sale_order.partner_id.id != partner.id and request.website.partner_id.id != partner.id :
                ...
                partner_invoice_id = request.env['res.partner'].sudo().search(
                    [('type', '=', 'invoice'), ('parent_id', '=', partner.id)])
                if not partner_invoice_id:
                    partner_invoice_id = partner
                sale_order.write({'partner_invoice_id': partner_invoice_id.id})
            ...
            if update_pricelist and sale_order.state == 'draft':
                request.session['website_sale_current_pl'] = pricelist_id
                values = {'pricelist_id': pricelist_id}
                sale_order.write(values)
