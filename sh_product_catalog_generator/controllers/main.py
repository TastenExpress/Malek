...
class DownloadCustomer(http.Controller):
    ...
    def download_catalog(self,**kw):
        if request.httprequest.method == 'POST':
            ...
            datas = {'id': 5, 'catalog_type': 'category', 'product_ids': [], 'category_ids': product_cats, 'price': True, 'pricelist_id':(pricelistobj.id,pricelistobj.name) , 'image': True, 'image_size': 'medium', 'description': True, 'product_link': True, 'style': kw['style'], 'int_ref': True, 'style_box': '2', 'break_page': False, 'break_page_after_products': 4,'on_hand':True, 'currency_id': (2, 'USD'), '__last_update': datetime.datetime(2021, 12, 16, 17, 43, 54, 460597), 'display_name': 'product.catalog.wizard,5', 'create_uid': (2, 'Mitchell Admin'), 'create_date': datetime.datetime(2021, 12, 16, 17, 43, 54, 460597), 'write_uid': (2, 'Mitchell Admin'), 'write_date': datetime.datetime(2021, 12, 16, 17, 43, 54, 460597)}
            if kw['style']=='style_5':
                datas['break_page']=4
